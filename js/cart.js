...
function addToCart(pId, qty = 1) {
  const p = (window.STORE_PRODUCTS || []).find(x => x.id === pId) || (window.DEFAULT_PRODUCTS || []).find(x => x.id === pId);
  if (!p) {
  ...
  if (existing) {
    ...
    CART.push({ id: p.id, nm: p.nm, pr: p.pr, img: p.img, q: qty });
  }
  saveCartToStorage();
  ...
  if (cpItems) {
    if (!CART.length) {
      ...
      cpItems.innerHTML = CART.map(item => `
        <div class="cpitem">
          <div class="cpth">${item.img ? `<img src="${item.img}" alt="${item.nm}">` : `<div style="background:#eee;width:100%;height:100%;display:flex;align-items:center;justify-content:center">🛍️</div>`}</div>
          <div style="flex:1">
            <div class="cpnm">${item.nm}</div>
            <div class="cppr">₹${item.pr * item.q}</div>
            <div class="cpqty">
              <button class="qbtn" onclick="updateCartQty('${item.id}', -1)">−</button>
